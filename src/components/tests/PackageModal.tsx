  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-white dark:bg-slate-800/95 rounded-2xl shadow-2xl dark:shadow-black/50 p-6 w-full max-w-4xl relative max-h-[90vh] overflow-y-auto border border-slate-200 dark:border-slate-700"
      ...
      >
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSubmit(e as any); }} className="space-y-6">
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Paket Adı</label>
            <Input
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">İndirim (%)</label>
            <Input
              type="number"
              placeholder="0"
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">Test Seçimi</label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Sol: Mevcut Testler */}
              <div className="border border-slate-200 dark:border-slate-600 rounded-xl p-3">
                ...
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {tests.map((test) => (
                    <label key={test.id} className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${
                      ...
                      />
                      <div className={`flex items-center gap-3 flex-1 ${formData.selectedTestIds.has(test.id) ? 'opacity-100' : 'opacity-70'}`}>
                        ...
                        <div className="flex-1">
                          <span className="font-medium text-slate-900 dark:text-white text-sm">{test.name}</span>
                ...
                {formData.selectedTestIds.size === 0 ? (
                  ...
                  <div className="space-y-2 max-h-80 overflow-y-auto">
                    {Array.from(formData.selectedTestIds).map((testId) => {
                      ...
                      return test ? (
                        <div key={testId} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg border border-slate-200 dark:border-slate-600">
                          <div className="flex items-center gap-3">
                            <div className={`w-8 h-8 rounded-lg ${categoryColors[test.category]?.bg} ${categoryColors[test.category]?.darkBg} flex items-center justify-center ${categoryColors[test.category]?.icon} ${categoryColors[test.category]?.darkIcon}`}>
                              {categoryIcons[test.category] || <Beaker size={16} />}
                            </div>
                            ...
                            <div className="flex items-center gap-2 mt-1">
                              ...
                              <div className="flex items-center gap-1">
                                <input
                                  ...
                                  className="w-20 px-2 py-1 text-xs border border-slate-200 dark:border-slate-600 rounded bg-white dark:bg-slate-800 text-slate-900 dark:text-white"
                                />
          ...
          <Button variant="primary" className="w-full bg-gradient-to-r from-indigo-500 to-purple-500 hover:from-indigo-600 hover:to-purple-600 shadow-lg shadow-indigo-500/20">
